...
def friendsfeed(request):
    ...
    for p in posts:
        c_count = Comment.objects.filter(post=p).count()
        l_count = Like.objects.filter(post=p).count()
        comment_count_list.appent(c_count)
        like_count_list.append(l_count)
    zipped_list = zip(posts,comment_count_list,like_count_list)
...
def friends(request):
    ...
    all_profiles = Profile.objects.exclude(user=request.user).exclude(id__user=user_friends_profiles).exclude(id__in=request_sent_profiles)
